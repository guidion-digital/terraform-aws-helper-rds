...
import logging
import os
import dependencies.pymysql as pymysql
...
def handler(event, context):  # type: ignore
    ...
    if step == "createSecret":
        ...
        logger.error("handler: Invalid step parameter %s for secret %s" % (step, arn))
        raise ValueError("Invalid step parameter %s for secret %s" % (step, arn))


def create_secret(service_client, arn, token):  # type: ignore
...
def set_secret(service_client, arn, token): # type: ignore
    ...
    try:
        with conn.cursor() as cur:
            ...
            ver = cur.fetchone()
            password_option = get_password_option(ver[0]) # type: ignore
...
def test_secret(service_client, arn, token): # type: ignore
...
def finish_secret(service_client, arn, token): # type: ignore
...
def get_connection(secret_dict): # type: ignore
...
def get_ssl_config(secret_dict): # type: ignore
...
def connect_and_authenticate(secret_dict, port, dbname, use_ssl): # type: ignore
...
def get_secret_dict(service_client, arn, stage, token=None): # type: ignore
...
def get_password_option(version): # type: ignore
...
def get_environment_bool(variable_name, default_value): # type: ignore
...
def get_random_password(service_client) -> str: # type: ignore
